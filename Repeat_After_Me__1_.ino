...
#define ROUNDS_TO_WIN      12 //Number of rounds to succesfully remember before you win. 13 is do-able.
...
// Light an LED for buzz_length_ms (previously also played a tone on the buzzer)
void toner(byte which, int buzz_length_ms)
{
  setLEDs(which); //Turn on a given LED

  delay(buzz_length_ms); // Keep the LED lit for the same duration the tone used to play

  setLEDs(CHOICE_OFF); // Turn off all LEDs
}

// Play the winner light show (previously also played a tone on the buzzer)
...
// Play the loser light show (previously also played a tone on the buzzer)
...
// Do nothing but show a beegees-style light show (previously also played music on the buzzer)
...
void play_beegees()
{
  ...
  delay(1000); // Wait a second before starting the light show

  while(checkButton() == CHOICE_NONE) //Run the light show until you press a button
  {
    // iterate through the light sequence:
